...
// !!! PERUBAHAN DI SINI: DARI 24 MENJADI 12 !!!
...
std::string private_key_to_nested_segwit_address(const std::vector<uint8_t>& private_key) {
    ...
    redeem_script[1] = 0x14; // OP_PUSHBYTES_20
...
std::string generate_mnemonic_bip39(uint32_t seed_value, const std::vector<std::string>& wordlist) {
    ...
    // 16 byte * 8 bit/byte / 32 = 4 bit checksum (Sesuai untuk 12 kata)
    ...
    for (size_t i = 0; i < MNEMONIC_WORD_COUNT; ++i) { // Loop 12 kali
        uint16_t word_index = 0;
        for (int bit = 10; bit >= 0; --bit) { // Ambil 11 bit per kata
...
// --- FUNGSI DIPERBARUI UNTUK MENGHASILKAN BANYAK ALAMAT ---
// Mengembalikan vector dari tuple: (legacy, nested, native, wif, path)
...
generate_all_addresses_and_private_key(const std::string& mnemonic, int num_addresses) {
    ...
    // Path derivasi default BIP44 untuk Bitcoin, digunakan oleh Trust Wallet
    const std::string base_derivation_path = "m/44'/0'/0'/0/"; // Basis path
    ...
    // 2. Seed -> Master Key (BIP32) - Dilakukan sekali
    ...
    for (int i = 0; i < num_addresses; ++i) {
        ...
        try {
            ...
            // 5. Hasilkan alamat dan WIF dari private key final
            ...
            // Tambahkan ke vector hasil
            results.emplace_back(legacy_address, nested_segwit_address, native_segwit_address, private_key_wif, current_path);
...
void run_parallel_generation(
    ...
    int num_addresses // Parameter baru
) {
    ...
    for (int i = 0; i < num_processes; ++i) {
        ...
        if (pid == -1) {
            ...
            // 'i' adalah ID child (0 hingga num_processes - 1)
            ...
            if (!outfile_child) {
                ...
                exit(1); // Keluar dari child process
            }
            ...
            if (child_start_ts <= end_timestamp) {
                ...
                if (offset != i) {
                    ...
                    if (child_start_ts > std::numeric_limits<uint32_t>::max() - adjustment) {
                        child_start_ts = end_timestamp + 1; // Mencegah overflow, set agar loop tidak berjalan
                    } else {
            ...
            for (uint32_t ts = child_start_ts; ts <= end_timestamp; ts += num_processes) {
                try {
                    ...
                    // --- PERUBAHAN UPGRADE ---
                    // Panggil fungsi yang diperbarui untuk mendapatkan vector
                    auto address_list = generate_all_addresses_and_private_key(mnemonic, num_addresses);
                    
                    // Loop melalui hasil dan tulis ke file
                    // Format: [legacy] [nested] [native] [wif] [path]
                    for (const auto& addresses : address_list) {
                        outfile_child << std::get<0>(addresses) << " " // legacy
                                      << std::get<1>(addresses) << " " // nested
                                      << std::get<2>(addresses) << " " // native
                                      << std::get<3>(addresses) << " " // wif
                                      << std::get<4>(addresses) << "\n"; // path
                    }
                    // --- AKHIR PERUBAHAN UPGRADE ---

                } catch (const std::exception& e) {
                    // Tangani error kritis (e.g., master key generation)
            ...
            exit(0); // --- PENTING: Child harus keluar ---
        } else {
    ...
    for (int i = 0; i < num_processes; ++i) {
        ...
        if (pid > 0) {
            ...
            if (WIFEXITED(status) && WEXITSTATUS(status) != 0) {
                std::cerr << "Child process " << pid << " (index " << i << ") exited with error." << std::endl;
                all_ok = false;
            }
        } else {
            all_ok = false; // fork() gagal untuk child ini
        }
    }
    ...
    for (int i = 0; i < num_processes; ++i) {
        ...
        unlink(child_progress_file.c_str()); // Hapus file progress
    }

    final_outfile.close();
    std::cout << "Generation complete. All addresses and private keys saved to " << base_output_file << "\n";
}
...
int main() {
    ...
    std::cout << "--- Mnemonic Set: 12 Words ---" << std::endl; // Info perubahan
    std::cout << "--- NOW WITH BIP32/BIP44 (Trust Wallet Compatible) ---" << std::endl;
    ...
    // Minta jumlah alamat SETELAH memilih mode
        ...
        if (outfile.is_open()) {
            ...
            try {
                ...
                for (const auto& addresses : address_list) {
                    ...
                    std::string path = std::get<4>(addresses);

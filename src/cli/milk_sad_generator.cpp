...
// --- DIPERBARUI UNTUK MENGGUNAKAN BIP32/BIP44 (TRUST WALLET PATH) ---
std::tuple<std::string, std::string, std::string, std::string> generate_all_addresses_and_private_key(const std::string& mnemonic) {
    ...
    const std::string derivation_path = "m/44'/0'/0'/0/0";
    ...
    HDKey derived_key = derive_key_from_path(master_key, derivation_path);
        ...
        if (pid == -1) {
            ...
            for (uint32_t ts = child_start_ts; ts <= end_timestamp; ts += num_processes) {
                try {
                    ...
                    outfile_child << std::get<0>(addresses) << " " 
                                  << std::get<1>(addresses) << " " 
                                  << std::get<2>(addresses) << " " 
                                  << std::get<3>(addresses) << "\n";
                } catch (const std::exception& e) {
                    // Tangani error derivasi (meskipun jarang terjadi untuk path tetap)
                    std::cerr << "Process " << i << " skipped timestamp " << ts << " due to derivation error: " << e.what() << std::endl;
                }
...
int main() {
    ...
    if (choice == 1) {
        // --- PILIHAN 1 (SINGLE) - TIDAK BERUBAH ---
        ...
        if (outfile.is_open()) {
            ...
            try {
                ...
                outfile << "Private Key (WIF) (Path m/44'/0'/0'/0/0): " << private_key << std::endl;
                ...
                std::cout << "Private Key (WIF) (Path m/44'/0'/0'/0/0): " << private_key << std::endl;
        ...
        run_parallel_generation(start_timestamp, end_timestamp, output_file, PROGRESS_FILE_RANGE, wordlist);
    }
    else if (choice == 3) {
        ...
        run_parallel_generation(start_timestamp, end_timestamp, output_file, PROGRESS_FILE_FULL, wordlist);
    }
